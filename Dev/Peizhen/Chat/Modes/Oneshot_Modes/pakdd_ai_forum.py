...
llm_decider_mode = system.import_library("../../../../../HB3/chat/modes/llm_decider_mode.py")
mode = system.import_library("../../../../../HB3/chat/modes/mode.py")
llm_model = system.import_library("../../../../../HB3/lib/llm/llm_models.py")
...
system_actions = system.import_library("../../../../../HB3/chat/actions/system_actions.py")
interaction_actions = system.import_library("../../../../../HB3/chat/actions/interaction_actions.py")
vision_actions = system.import_library("../../../../../HB3/chat/actions/vision_actions.py")
custom_actions = system.import_library("../../Actions/custom_actions.py")  
...
PERSONA_UTIL = system.import_library("../../../../../HB3/lib/persona_util.py")
# PERSONA_UTIL = system.import_library("../../Lib/custom_persona_util.py")

MODE_NAME = "pakdd_aiforum_interaction"

system_message_template_idle = """
{PERSONALITY}
You will also be provided with the conversation up until now.
{INTERACTION_HIDDEN_SYS_MSG}
Speak in English unless otherwise requested.
"""
SUMMARY_STEP = 100  # summary every 100 speeches


class PakddInteractionMode(llm_decider_mode.LLMDeciderMode):
    """The interaction mode customized for the AI forum at PAKDD 2025."""

    DECISION_MODEL = None

    def on_mode_entry(self, from_mode_name: Optional[str]):
        """Called when you enter custom interaction mode.
        Args:
        ...
        if from_mode_name is not None:
            system.messaging.post("tts_say", ["Entering custsom interaction mode", "eng"])
    ...
    def create_decision_mode_talking(self):
        ...
        base_system_prompt = """
        Your job is to analyse the conversation and decide what to do with the current user input by calling one of the functions.
        Speak in English unless otherwise requested.
        """
    ...
    def should_respond(self, text):
        if text is None:
            return False
        if "what do you think" in text.lower() or 'final statement' in text.lower():  # "Ameca" in text and 
            return True
        return False

    async def manage_context(self):
        if len(ROBOT_STATE.interaction_history) % SUMMARY_STEP == 0:
            history = await self.decision_model_chat.get_message_prompt()
            print(f'decision model history: \n {len(history)}')
            user_message = await self.decision_model_chat.get_message_prompt() # a list of dict
            msg_str = '\n'.join([msg['content'][0].get('text', '') for msg in user_message])
            
            messages = [
            {"role": "system", 
             "content": [{"type": "text", "text": """
                You are participating an AI forum at PAKDD 2025, where the moderator is Ms Elizabeth Whitelock, CEO, Director, MQ Adjunct Fellow.
                Now your task is summarizing context provided by the users"
                """},]
            },
            {"role": "user",
             "content": [{"type": "text","text": msg_str,},]
            },
            ]
            summary = await llm_interface.run_chat(
                model="gpt-4o", messages=messages)
            system.messaging.post("speech_recognized", {"speaker": "Lizy", "text": f"summary so far: {summary}"})
            # system.messaging.post("speech_recognized", {"speaker": "Lizy", "text": "what do you think"})  # TODO test
    ...
    async def on_message(self, channel: str, message: Any):
        ...
        if channel == "speech_recognized":
            if not robot_state.speaking:
                ...
                if not self.should_respond(message.get("text")):
                    print("[PAKDD AI Forum]: Keep Silent!")
                    # history = await self.decision_model_chat.get_message_prompt()

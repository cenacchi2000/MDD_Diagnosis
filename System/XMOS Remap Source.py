"""
Creates a remapped pulseaudio source which only grabs the first channel from the XMOS.
Only applies to XVF3500
...
MASTER_SOURCE = "alsa_input.usb-XMOS_XMOS_VocalFusion_St__UAC1.0_-00.multichannel-input"
REMAPPED_SOURCE = "XMOS-Mono"
...
async def run_subprocess(*args):
    ...
    if process.returncode != 0:
        print("pactl failed!", await process.stderr.read(4096))
        return None
    return process
        ...
        process = await run_subprocess(
            ...
            " ".join(
                [
                    ...
                    "rate=48000",  # For some reason when using remix=false, we have to specify the rate manually
                    "format=s16le",  # This should be implied, but including for completeness
                ]
            ),
        )
        if not process:
